...
async function searchApk(query, limit = 10) {
    ...
    const url = `https://ws75.aptoide.com/api/7/apps/search?query=${encodeURIComponent(cleanQuery)}&limit=${limit + 5}`;
    const res = await axios.get(url, { timeout: 15000 });
    let list = res.data?.datalist?.list || [];
    
    // Filter out Aptoide launcher store itself unless user explicitly searched for aptoide
    if (!cleanQuery.toLowerCase().includes('aptoide')) {
 ...
 * Handles App IDs, package names, URLs, and search names,
 * while preventing false fallbacks to the Aptoide app itself.
 * @param {string|number} appIdOrPackageOrUrl 
 */
async function getApkDetails(appIdOrPackageOrUrl) {
    if (!appIdOrPackageOrUrl) throw new Error('App ID, Package name, or URL is required');
    let target = String(appIdOrPackageOrUrl).trim();

    // 1. If user provided an Aptoide or web URL, extract package name or ID
    if (target.startsWith('http://') || target.startsWith('https://')) {
        const aptoideSubdomain = target.match(/https?:\/\/([a-zA-Z0-9_\.\-]+)\.en\.aptoide\.com/);
        if (aptoideSubdomain && aptoideSubdomain[1]) {
            target = aptoideSubdomain[1];
        } else {
            const urlAppId = target.match(/[?&]app_id=(\d+)/) || target.match(/app_id=(\d+)/);
            if (urlAppId && urlAppId[1]) {
                target = urlAppId[1];
            } else {
                const pkgMatch = target.match(/([a-zA-Z0-9_]+\.[a-zA-Z0-9_]+\.[a-zA-Z0-9_\.]+)/);
    ...
    // 2. Lookup by numeric App ID
    if (/^\d+$/.test(target)) {
        try {
            const res = await axios.get(`https://ws75.aptoide.com/api/7/app/get/app_id=${target}`, { timeout: 15000 });
            data = res.data?.nodes?.meta?.data || res.data?.datalist?.list?.[0];
        } catch (e) {}
    }

    // 3. Lookup by Package name (contains dot and no spaces)
    if (!data && target.includes('.') && !target.includes(' ')) {
        try {
            const res = await axios.get(`https://ws75.aptoide.com/api/7/app/get/package_name=${encodeURIComponent(target)}`, { timeout: 15000 });
            data = res.data?.nodes?.meta?.data;
        } catch (e) {}

        if (!data) {
            try {
                const res = await axios.get(`https://ws75.aptoide.com/api/7/apps/get/package_name=${encodeURIComponent(target)}`, { timeout: 15000 });
    ...
    // 4. Fallback: Search by clean query
    if (!data) {
        const cleanQuery = target.replace(/https?:\/\/\S+/g, '').replace(/[^a-zA-Z0-9\s._-]/g, ' ').trim() || target;
        const res = await axios.get(`https://ws75.aptoide.com/api/7/apps/search?query=${encodeURIComponent(cleanQuery)}&limit=10`, { timeout: 15000 });
        const list = (res.data?.datalist?.list || []).filter(item => {
            if (!cleanQuery.toLowerCase().includes('aptoide') && (item.package === 'cm.aptoide.pt' || item.name?.toLowerCase() === 'aptoide')) {
                return false;
            }
            return true;
        });
    ...
    if (!data) {
        throw new Error(`App not found for "${appIdOrPackageOrUrl}".`);
    }
