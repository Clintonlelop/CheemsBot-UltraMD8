...
function repairSessionFolder(sessionDir = './session', options = {}) {
    ...
    try {
        ...
        let firstUnuploadedPreKeyId = null;

        const credsPath = path.join(sessionDir, 'creds.json');
        if (fs.existsSync(credsPath)) {
            try {
                const creds = JSON.parse(fs.readFileSync(credsPath, 'utf8'));
                if (typeof creds.firstUnuploadedPreKeyId === 'number') {
        ...
        for (const file of files) {
            ...
            try {
                ...
                // Option to prune old/consumed pre-keys to prevent Signal session desync
                if (options.prunePreKeys && file.startsWith('pre-key-')) {
                    const keyId = parseInt(file.replace('pre-key-', '').replace('.json', ''));
                    const ageMs = Date.now() - stats.mtimeMs;

                    // If keyId is far below firstUnuploadedPreKeyId, it is already consumed by previous sessions
                    if (firstUnuploadedPreKeyId && !isNaN(keyId) && keyId < (firstUnuploadedPreKeyId - 25)) {
                        fs.unlinkSync(filePath);
                        removedCount++;
                        continue;
                    }

                    // Also prune pre-keys older than 24h if there are many pre-keys
