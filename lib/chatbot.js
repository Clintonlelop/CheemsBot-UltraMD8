...
const { textToPttVoiceNote } = require('./media-tools');
...
4. You are operating inside a WhatsApp bot, but right now you are conversing naturally as a friend.`;
    ...
    try {
        const fullPrompt = `${senderName}: ${userMessage}`;
        ...
        console.log('[Chatbot askGemini error]', err?.message || err);
    }
 ...
 * Returns { buffer, waveform } formatted strictly for WhatsApp PTT playback.
 */
async function generateVoiceNoteBuffer(text, lang = 'en') {
    if (!text) return null;
    return await textToPttVoiceNote(text, lang);
}
