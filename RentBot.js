...
try {
  ...
  if (fs.existsSync(ownerPath)) {
    owner = JSON.parse(fs.readFileSync(ownerPath).toString())
  }
} catch (e) {}
...
try {
const kay = chatUpdate.messages ? chatUpdate.messages[0] : null
if (!kay || !kay.message) return
kay.message = (Object.keys(kay.message)[0] === 'ephemeralMessage') ? kay.message.ephemeralMessage.message : kay.message
if (kay.key && kay.key.remoteJid === 'status@broadcast') return
if (!XeonBotInc.public && !kay.key?.fromMe && chatUpdate.type === 'notify') return
if (kay.key?.id && typeof kay.key.id === 'string' && kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
const m = smsg(XeonBotInc, kay, store)
// Legacy XeonCheems7 handler was not part of the supplied source tree; keep rented socket alive.
// Rented-bot message handling is intentionally disabled rather than crashing.
